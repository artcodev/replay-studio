...
from app.providers.api_football_mapping import (
...
def test_bundle_uses_embedded_fixture_data_and_normalizes_complete_lineup() -> None:
    ...
    provider = ApiFootballProvider(client=FakeApiFootballClient(fake_get))
...
def test_timeline_ids_are_signature_based_and_stable_when_rows_reorder() -> None:
    ...
    original, _ = map_timeline([first, second], event)
    reordered, _ = map_timeline([second, first], event)
...
def test_roster_quality_requires_a_starting_goalkeeper_for_each_team() -> None:
    ...
    players, lineup = map_lineups(fixture["lineups"], event)
    timeline, substitutions = map_timeline(fixture["events"], event)

    quality = assess_roster_quality(players, lineup, event, substitutions)
...
def test_bundle_retains_fixture_when_optional_lineup_request_fails() -> None:
...
def test_search_resolves_team_pair_then_uses_head_to_head() -> None:
    calls: list[tuple[str, dict]] = []
...
def test_search_requires_an_unambiguous_team_pair() -> None:
    async def unexpected_get(
        ...
        raise AssertionError("invalid search must not call the transport")

    provider = ApiFootballProvider(client=FakeApiFootballClient(unexpected_get))
