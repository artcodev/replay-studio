...
| Ручное редактирование | Реализованы frame-person annotations, confirm/ignore/merge/split/bind, manual ball timeline, selected-object path, player-action timeline и durable manual player trajectory (`TD-TRACK-01` закрыт) | Automatic actions — `TD-ACT-01` |
...
### TD-TRACK-01 — закрыт: ручное перемещение игрока durable

**Решено.** Drag публикуется выделенной командой
`PUT /scenes/{id}/tracks/{trackId}/trajectory`
([track_trajectory_corrections.py](../apps/api/app/track_trajectory_corrections.py)).
Правка хранится sparse в `payload.trackTrajectoryCorrections` и привязана к
`canonicalPersonId` — идентичности, которую `assign_persistent_canonical_person_ids`
держит стабильной между прогонами, а не к временному render track ID.
Read-model (`reconstruction_series_window`) накладывает ручные точки поверх
derived trajectory как авторитетные, помечая их `positionSource: manual`, поэтому
review никогда не примет пользовательскую позицию за наблюдение модели. Трек без
canonical identity получает fail-closed отказ вместо правки, которую следующий
rebuild молча потерял бы. Dense keyframes через общий Scene PUT больше не
теряются: сам общий Scene PUT удалён (см. п.9 инвариантов).
...
9. Пользовательские правки сцены публикуются только выделенными командами
   (title, event-bindings, track metadata, segment layout, ball, actions,
   идентичность, калибровка). Generic `PUT /scenes/{id}` удалён: клиент
   никогда не гоняет документ целиком, поэтому ревизия документа охраняет
   программные записи, а не спорит с редактором.
10. Обратная совместимость со старыми локальными данными не поддерживается
