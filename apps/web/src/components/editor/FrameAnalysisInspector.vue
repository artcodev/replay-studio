  ...
  <div class="frame-analysis-card" :class="{ stale: !active }">
    ...
    <div class="frame-detection-list">
      <button v-for="person in analysis.people" :key="person.id" :class="{ selected: person.id === selectedPersonId && active }" @click="emit('select-person', person)">
        ...
        <span><strong>{{ personLabel(person) }}</strong><small>{{ person.previewState === 'merged' ? `merge → ${person.mergeTargetId} · ` : person.previewState === 'split' ? `split [${person.rangeStart?.toFixed(2)}, ${person.rangeEnd?.toFixed(2)}) · ` : person.previewState === 'confirmed' ? 'confirmed · ' : '' }}{{ person.kind ? `${person.kind} · ` : '' }}{{ personCanonicalId(person) ? `${personCanonicalId(person)} · ` : '' }}{{ person.pitch ? `x ${person.pitch.x.toFixed(1)} · z ${person.pitch.z.toFixed(1)}` : 'no 3D position' }}{{ person.matchDistance !== null ? ` · Δ${person.matchDistance.toFixed(1)}m` : '' }} <em v-if="metricBadge(person)" :class="{ uncertain: metricBadge(person) === 'UNCERTAIN' }">{{ metricBadge(person) }}</em></small></span>
