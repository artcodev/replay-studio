...
function finiteManualTime(value: number, fallback = 0) {
  return Number.isFinite(value) ? value : fallback
}

export function clampManualBallTime(value: number, duration: number) {
  const safeDuration = Math.max(0, finiteManualTime(duration))
  return Math.round(Math.min(safeDuration, Math.max(0, finiteManualTime(value))) * 1000) / 1000
}
...
function sameManualBallTime(first: number, second: number) {
  return Math.abs(first - second) < MANUAL_BALL_TIME_STEP / 2
}

/** Pure action resolver shared by the UI and unit tests. */
export function manualBallTimelineEvents(
  ...
  const duration = Math.max(0, finiteManualTime(state.duration))
  ...
  if (action.type === 'add') {
    ...
    const duplicate = times.find((candidate) => sameManualBallTime(candidate, time))
  ...
  const selectedIndex = times.findIndex((time) => sameManualBallTime(time, selected))
  ...
  return sameManualBallTime(from, to) ? [] : [{ type: 'updateTime', value: { from, to } }]
}
</script>
