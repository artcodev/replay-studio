<script lang="ts">
import type { PlayerAction, PlayerActionKeypointKind, PlayerActionType } from '../types'
...
export type PlayerActionLayoutItem = {
  action: PlayerAction
  lane: number
}
...
/**
 * Makes persisted and suggested actions safe to render and edit. Imported data
 * may contain inverted boundaries; those are ordered once during normalization.
 */
...
/** Pure reducer used by the form and unit tests. It never mutates its input. */
export function reducePlayerAction(action: PlayerAction, edit: PlayerActionEdit, duration: number): PlayerAction {
  ...
  // Automatic hypotheses are evidence to review, not editor-owned records.
  if (current.source !== 'manual') return current

  if (edit.type === 'set-type') return { ...current, type: edit.value }

  if (edit.type === 'set-start') {
    const minimumInterval = Math.min(PLAYER_ACTION_TIME_STEP, safeDurationForAction(duration))
    const startTime = Math.min(current.endTime - minimumInterval, clampPlayerActionTime(edit.time, duration))
    return normalizePlayerAction({ ...current, startTime }, duration)
  }

  if (edit.type === 'set-end') {
    const minimumInterval = Math.min(PLAYER_ACTION_TIME_STEP, safeDurationForAction(duration))
    const endTime = Math.max(current.startTime + minimumInterval, clampPlayerActionTime(edit.time, duration))
    return normalizePlayerAction({ ...current, endTime }, duration)
  }
...
function safeDurationForAction(duration: number) {
  return Math.max(0, finiteActionNumber(duration))
}

/** Greedy lane assignment keeps overlapping action intervals readable. */
