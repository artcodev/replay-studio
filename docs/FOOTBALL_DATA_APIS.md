...
Для текущего некоммерческого прототипа основной источник состава — API-Football, а уже интегрированный TheSportsDB остаётся альтернативным каталогом. API-Football даёт полный match-day lineup и события на бесплатном тарифе, тогда как TheSportsDB free намеренно обрезает оба ответа до пяти строк. Внутренний контракт остаётся провайдер-независимым. UI передаёт provider явно; ни явный запрос, ни сохранённый fixture/event ID не переключаются на другой источник, потому что пространства ID несовместимы. Для проверки координат и алгоритмов используем открытые StatsBomb, SkillCorner и Metrica офлайн. Sportmonks имеет наибольшую потенциальную ценность среди доступных коммерческих API благодаря `ballCoordinates`, но это данные только для выбранных матчей/соревнований и не замена реконструкции из нашего видео.

Приоритеты:

1. API-Football — реализовать основным адаптером для полного состава и таймлайна.
2. TheSportsDB — оставить явно выбираемой альтернативой для поиска матча и базовых метаданных.
...
Практическая схема: API-Football становится primary, а TheSportsDB остаётся отдельным выбираемым адаптером. В project-level snapshot сохраняются нормализованные event, teams, players, lineup, timeline, substitutions, roster quality, warnings и `fetchedAt`. Raw response и его hash сейчас в scene не сохраняются; upstream JSON живёт только во временном memory/Redis cache. Повторная синхронизация не должна перетирать ручные назначения пользователя. Перед допуском состава к автоматической идентификации текущая реализация проверяет по фактическому ответу 11 стартеров на команду, уникальность player IDs через дедупликацию и присутствие участников замен в lineup. Отдельная проверка наличия вратаря пока не реализована.

## Выбор провайдера и ключи

Backend регистрирует оба адаптера одновременно. `MATCH_DATA_PROVIDER` задаёт
предпочтительный default для запроса, который не передал provider явно; UI
может выбрать другой provider для каждого catalog/bind-запроса. Ключи остаются
на сервере и никогда не передаются во Vue:
...
делает недоступным явно выбранный API-Football adapter. Для совместимости
старый запрос без поля `provider` может разрешить default в другой
сконфигурированный adapter, если preferred adapter вообще не настроен. Это
единственное автоматическое разрешение: явный provider, ошибка upstream и
сохранённый `eventId` никогда не переключаются. `GET /api/catalog/providers`
возвращает `preferredProvider`, фактически разрешённый `defaultProvider` и
состояние доступности адаптеров без ключей. При смене provider проект нужно
заново привязать через его каталог: сохранённый `eventId` можно отправлять
только адаптеру, указанному в `source` того же snapshot. Refresh всегда
использует сохранённый `source`, а не текущий UI/default provider.
...
`GET /api/catalog/providers` сейчас означают наличие серверного ключа, а не
...
Нельзя протащить поля конкретного API напрямую в UI и реконструкцию. Внутренний snapshot должен сохраняться рядом со сценой и оставаться работоспособным при недоступности провайдера.
...
- хранить `provider`, исходные ID и `fetchedAt`;
...
- нормализованный snapshot хранится в проекте; raw response/hash в scene пока не сохраняются и остаются отдельным диагностическим техдолгом;
...
- не менять реконструированный трек только из-за обновления имени или состава.
...
- Использовать общий интерфейс `MatchDataProvider`, оставив TheSportsDB отдельным выбираемым каталогом без cross-provider fallback.
- Использовать API-Football adapter за переменной окружения `API_FOOTBALL_API_KEY` и сделать его primary для полного lineup.
- Upstream response кешируется в memory/Redis, а нормализованный snapshot сохраняется в проекте; UI показывает provider availability, `rosterQuality` и warnings. Координатного coverage в текущем match-data contract нет.
- Source уже сохраняется у catalog event, bundle и project snapshot; более детальная provenance каждого identity suggestion остаётся техдолгом.
