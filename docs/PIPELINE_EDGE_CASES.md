...
| P0 | API/worker crashes after a run was queued or claimed | Persisted correction remains forever at `queued`/`processing`, or two API processes analyze the same run | Continuous bounded recovery monitor; process-independent owner lease with heartbeat/expiry; atomic queued/stale-processing claim; every publish is fenced by lease + run + input + revision. Legacy missing tokens are upgraded safely; corrupt input fingerprints become retryable failure rather than unsafe output | Progress resumes automatically after TTL; active leases are not stolen; a delayed old owner and duplicate terminal task are no-ops | Two-engine queued/reclaim races, heartbeat, active-not-stolen, old-owner fencing, terminal cleanup, legacy/corrupt recovery and concurrent seed tests |
...
| P1 | Legacy frame confirm conflicts with Bind/Unbind | Roster identity changes with annotation/frame order or bypasses uniqueness | New generic confirms cannot carry roster IDs; dedicated correction has authoring precedence over legacy values; incompatible dedicated states and ambiguous owners abort | Bind/unbind only from canonical inspector; generic editor changes role/label/identity graph | Same result for legacy-before/after/same-frame; zero duplicate roster owners; split-local baseline |
...
- **Split**: break a canonical identity over a half-open `[start, end)` range.
  ...
  cannot-link barrier prevents the automatic/legacy merge pass from reconnecting
...
3. Keep base sampled-frame person/legacy-ball artifacts limited to pre-annotation
