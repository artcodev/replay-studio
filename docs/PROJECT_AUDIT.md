...
- **Roboflow-путь калибровки** — удалён решением владельца (2026-07-22).
  PnLCalib является единственным автоматическим backend: при его
  недоступности Calibration завершается ошибкой и не публикует замену
  last-good artifact. Ручные anchors остаются отдельным явным вводом
  оператора, а temporal-граф переносит только уже принятые PnL/manual
  наблюдения.
...
4. Из [APPLICATION_IMPROVEMENT_PROJECT.md](APPLICATION_IMPROVEMENT_PROJECT.md)
   в работу взяты только три пункта (решение владельца, 2026-07-19):
   football-веса — shadow-замер (`benchmarks/detector_shadow_comparison.py`)
   и опция детектора `football.pt` в пайплайне (веса поставляет владелец,
   лицензионные риски он принял на себя и сам ведёт диалог с автором;
   классы резолвятся по именам, отсутствующий файл даёт явную ошибку при
   постановке в очередь), офлайн-сравнение BoT-SORT
   (`benchmarks/tracker_baseline.py`) и экспериментальный профиль
   `contactPointProfile=pose-feet` (стопы RTMPose вместо низа bbox,
   визуальная приёмка). Остальные разделы документа остаются гипотезами
   и не имплементируются без отдельного решения.
